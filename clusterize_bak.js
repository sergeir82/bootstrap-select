...
;(function(name, definition) {
  ...
  var Clusterize = function(data) {
    ...
    scrollEv = function() {
      ...
      console.log(last_cluster, self.getClusterNum());
      ...
      var clust = self.options.scroll_top / (self.options.cluster_height - self.options.block_height);
      var part = clust - Math.floor(clust);
      var height = 0;

      var $li = $(self.content_elem).find('li:not(.clusterize-extra-row)');
      $li.each(function() {
        //console.log(this);
      ...
      var bn = self.options.blocks_in_cluster;
      var a = (height - self.options.cluster_height) / self.options.cluster_height;

      console.log('height', height,  self.options.cluster_height);

      //Math.floor(this.options.scroll_top / (this.options.cluster_height - this.options.block_height))

      console.log('a', a, self.options.scroll_top, (self.options.cluster_height - self.options.block_height) * (last_cluster));
      var top = - a * (self.options.scroll_top - (self.options.cluster_height - self.options.block_height) * (last_cluster));
      console.log('top', top);
